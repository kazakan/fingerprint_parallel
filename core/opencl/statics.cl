long __sum(__local long *values) {
...
float __sumf(__local float *values) {
    const int local_id = get_local_id(0);
    const int group_size = get_local_size(0);
 ...
 */
__kernel void sum(__global uchar *input, __global long *output,
                  __local long *tmp, int inputSize) {
 ...
 */
__kernel void squareSum(__global uchar *input, __global long *output,
                        __local long *tmp, int inputSize) {
    int global_id = get_global_id(0);
    int local_id = get_local_id(0);
    int group_id = get_group_id(0);

    long val = input[global_id];
    val *= val;
